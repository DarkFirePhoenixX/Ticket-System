    ...
    <form method="post" action="{{ route('tickets.update', $ticket->id) }}">
      <div class="form-group fw-semibold">
        ...
        <select class="form-select" name="line_number" id="lineNumber">
          <option selected name="line_number" value="{{ $ticket->line_number }}">{{ $ticket->line_number }}</option>
          <option name="line_number" value="8632 [F]">8632 [F]</option>
      ...
      <div class="form-group fw-semibold">
        ...
        <select class="form-select" id="from_departure" name="from">
          <option selected name="from" value="{{ $ticket->from }}">{{ $ticket->from }}</option>
          <option name="from" value="Blagoevgrad">Blagoevgrad</option>
      ...
      <div class="form-group fw-semibold">
        ...
        <select class="form-select" id="to_departure" name="to">
          <option selected name="to" value="{{ $ticket->to }}">{{ $ticket->to }}</option>
          <option name="to" value="Blagoevgrad">Blagoevgrad</option>
          <option name="to" value="Burgas">Burgas</option>
      ...
      <div class="form-group fw-semibold">
        ...
        <input type="date" class="form-control" name="departure_date" value="{{ $ticket->departure_date }}" id="datePicker">
      </div>
      </div>
      <div class="col-6">
      <div class="form-group fw-semibold">
        <label for="class">Class:</label>
         <select class="form-select" name="class" id="Class">
          <option selected name="class" value="{{ $ticket->class }}">{{ $ticket->class }}</option>
      ...
      <div class="form-group fw-semibold">
        ...
        <input type="text" class="form-control" placeholder="№ 1 to 100" name="seat" value="{{ $ticket->seat }}" id="seatShow" readonly>
      </div>
      <div class="form-group fw-semibold">
      ...
      <div class="form-group fw-semibold">
        ...
        <select class="form-select" aria-label="Default select example" name="discount" id="discounts">
        <option selected name="discount" value="{{ $ticket->discount }}">{{ $ticket->discount }}</option>
          <option name="discount" value="0">No discount</option>
      ...
      <br>
      <button type="submit" class="btn gradient-custom-2 text-light fw-semibold">Update ticket</button>
...
<script type="text/javascript">
 ...
 const from = document.getElementById('from_departure');
 const from_options = from.options;
 for (option of from_options) {
  if(from.value === option.value && option.index != 0) {
 ...
 const to = document.getElementById('to_departure');
 const to_options = to.options;
 for (option of to_options) {
  if(to.value === option.value && option.index != 0) {
    option.selected = true;
  break;
 }
 }
 ...
 const Class_options = Class.options;
 for (option of Class_options) {
  if(Class.value === option.value && option.index != 0) {
    option.selected = true;
  break;
 }
 }
 ...
 const lineNumber_options = lineNumber.options;
 for (option of lineNumber_options) {
  if(lineNumber.value === option.value && option.index != 0) {
    option.selected = true;
  break;
 }
 }
...
document.addEventListener('DOMContentLoaded',function(){
 ...
 const discounts_options = discounts.options;
 for (option of discounts_options) {
  if(discounts.value === option.value && option.index != 0) {
    option.selected = true;
  break;
 }
 }
...
document.getElementById('seatShow').addEventListener('click',function(){
  document.querySelectorAll('.takenSeats').forEach(takenSeat => {
    ...
    document.querySelectorAll('.seat').forEach(element => {
      if(element.innerText == document.getElementById('seatShow').value)
        element.classList.add('selected')
    });
  })
